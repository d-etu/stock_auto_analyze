    ...
    while True:
        for y in range(len(df_vol.columns)):
            ...
            for x in range(num, num+312):
                ...
                if any(new_data_vol_200[i] == max_value for i in range(200-date_length, 200)):
                    pass
                else:
                    continue
                ...
                dtw_end = function.dtw_distance(exist_data_end, new_data_end)
                if dtw_end > dtw_num or type(dtw_end) == type('a'):
                    continue
                ...
                dtw_vol = function.dtw_distance(exist_data_vol, new_data_vol)
                if dtw_vol > dtw_num or type(dtw_vol) == type('a'):
                    continue
                ...
                dtw_high = function.dtw_distance(exist_data_high, new_data_high)
                if dtw_high > dtw_num or type(dtw_high) == type('a'):
                    continue
                ...
                if x + int(date_length) < len(df.index):
                    ...
                    rate_return[1] = (df_high.iloc[x + int(date_length) + 1, y] /
                                      df.iloc[x + int(date_length) - 1, y]) * 100 - 100
                    info_save = function.save_xy(info_save, df, df_high, x, y, stock_amount, date_length, dtw_end,
                                                 dtw_vol, dtw_high, rate_return[0], rate_return[1])
                if stock_amount > 40:
                    break
            if stock_amount > 40:
                break
...
if __name__ == '__main__':
    ...
    if not info_save_total.empty:
        function.print_stock_analysis(stock_input, stock_amount_total-1, info_save_total,
                                  start_date, end_date, date_length, "dtw")
    end = time.time()
